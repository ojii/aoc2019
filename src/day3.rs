...
impl Segment {
    fn parse(from: &str) -> Segment {
        let (direction, length) = from.split_at(1);
        ...
        let len = length.parse::<i32>().unwrap();
        Segment { len, direction }
    }
}
...
impl Wire {
    fn new(from: &str) -> Wire {
        let path: Vec<Segment> = from.split(",").map(|part| Segment::parse(part)).collect();
        Wire { path }
    }
    ...
    fn points(&self) -> HashSet<Point> {
        ...
        let mut points = HashSet::with_capacity(
            self.path.iter().map(|segment| segment.len).sum::<i32>() as usize,
        );
        for segment in &self.path {
            let vector = segment.direction.as_vector();
